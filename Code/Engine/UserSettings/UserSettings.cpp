...
unsigned int eae6320::UserSettings::JustGetResolutionWidth()
{
	return s_resolutionWidth;
}

unsigned int eae6320::UserSettings::JustGetResolutionHeight()
{
	return s_resolutionHeight;
}
			...
			if ( lua_isnumber( &io_luaState, -1 ) )
			{
				...
				if ( IsNumberAnInteger( floatingPointResult ) )
				{
					if ( floatingPointResult >= lua_Number( 0 ) )
					{
						...
						eae6320::Logging::OutputMessage("The set resolution width is : %u",
							s_resolutionWidth); 
					}
					else
					{
			...
			if ( lua_isnumber( &io_luaState, -1 ) )
			{
				...
				if ( IsNumberAnInteger( floatingPointResult ) )
				{
					if ( floatingPointResult >= lua_Number( 0 ) )
					{
						...
						eae6320::Logging::OutputMessage("The set resolution height is : %u",
							s_resolutionHeight);
					}
					else
					{
