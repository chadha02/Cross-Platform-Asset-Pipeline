		...
		if ( fout.is_open() )
		{
				...
				"{\n";
			{
				fout << "vertices =\n"
					"\t{\n";
				{
					for (std::vector<sVertexInfo>::const_iterator it = i_vertexBuffer.begin(); it < i_vertexBuffer.end(); it++)
					{
						fout << "\t\t{\n";
						fout << "\t\tposition=\n"
							"\t\t\t{";
						{
							...
							fout << std::to_string(it->vertex.x) + "," + std::to_string(it->vertex.y) + "," + std::to_string(it->vertex.z) + ",";
							fout << "\n";
						...
						fout << "\t\tcolor =\n"
							"\t\t\t{";
						{
							fout << std::to_string(it->vertex.r) + "," + std::to_string(it->vertex.g) + "," + std::to_string(it->vertex.b) + "," + std::to_string(it->vertex.a) + ",";
							fout << "\n";

						}
						fout << "\t\t\t},\n";// Close color table
						fout << "\t\t},\n"; // close table for one vertex
					}
				}
					...
					"\t{\n";
				{
					for (std::vector<sVertexInfo>::const_iterator it = i_vertexBuffer.begin(); it < i_vertexBuffer.end(); it++)
					{
						fout << "\t\t" + std::to_string(it->vertex.u) + "," + std::to_string(it->vertex.v) + ",";
						fout << "\n";
					}
				}
				fout << "\t},\n";// Close UV table
				...
				fout << "\nindexes =\n"
					"\t{\n";
				{	int i = 1;
					for (std::vector<size_t>::const_iterator it = i_indexBuffer.begin(); it < i_indexBuffer.end(); it++)
					{
						fout <<std::to_string(*it) + ",";
						if (i % 3 == 0)
						{
							fout << "\n";							
						}
