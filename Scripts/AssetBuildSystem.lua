...
local function RegisterAssetToBeBuilt( i_sourceAssetRelativePath, i_assetType, i_optionalCommandLineArguments )
	...
	local registrationInfoGetBuilderRelativePath
	do
...
NewAssetTypeInfo( "effects",
	{
      ...
      if DoesFileExist( sourceAbsolutePath ) then
		...
		local path_vertexShader = effect.shaderPath.vertex
		local path_fragmentShader = effect.shaderPath.fragment
...
NewAssetTypeInfo( "materials",
	{
      ...
      if DoesFileExist( sourceAbsolutePath ) then
		...
		local path_texture = material.texturPath	
		if (path_texture == nil) then
			path_texture = "default.tga"
		end
